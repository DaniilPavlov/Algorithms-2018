...
public class JavaTasks {
    ...
    static public void sortSequence(String inputName, String outputName) throws IOException, IllegalFormatException {
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader(inputName))) {
            while ((currentLine = bufferedReader.readLine()) != null) {
                elem = Integer.parseInt(currentLine);
                sequenceList.add(elem);
                if (map.containsKey(elem)) {
                    valueOfFoundKey = map.get(elem);
                    map.put(elem, valueOfFoundKey + 1);
                    if (valueOfFoundKey + 1 > oftTimes) {
                        oftTimes = valueOfFoundKey + 1;
                        oftTimesElem = elem;
                    } else if (valueOfFoundKey + 1 == oftTimes) {
                        oftTimesElem = Math.min(oftTimesElem, elem);
                    }
                } else {
                    map.put(elem, 1);
                    if (oftTimes == 1)
        ...
        for (int i = 0; i < sequenceList.size(); i++) {
            currentElem = sequenceList.get(i);
            if (currentElem == oftTimesElem) continue;
            writer.write(String.valueOf(currentElem));
            writer.newLine();
        }
        for (int i = 0; i < oftTimes; i++) {
            writer.write(String.valueOf(oftTimesElem));
            writer.newLine();
        }
        writer.close();
    }
